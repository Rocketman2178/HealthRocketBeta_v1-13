...
export function SubscriptionPlan({ onOpenChange }: SubscriptionPlanProps) {
  ...
  const handlePlanClick = (plan: Plan) => {
    // If user is already on Preview Access and clicks Preview Access, just close
    if (currentPlan?.name === 'Preview Access' && plan.name === 'Preview Access') {
      setIsOpen(false);
      return;
    }
                    ...
                    return (
                      <div
                        ...
                        className={`w-full px-4 py-3 text-sm text-left hover:bg-gray-700 transition-colors ${
                          ...
                          plan.name === "Preview Access" || 
                          (currentPlan?.name !== "Preview Access" && !plan.comingSoon)
                            ? "cursor-pointer" 
                            : "cursor-not-allowed opacity-75"
                        }`}
                        onClick={() => {
                          if (plan.name === "Preview Access" || 
                              (currentPlan?.name !== "Preview Access" && !plan.comingSoon)) {
